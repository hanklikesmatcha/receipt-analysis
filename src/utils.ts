...
const logic = {
    readTextFromFile: async(client: any, file: object) => {
        ...
        for (const page in readResults) {
          ...
          if (result.lines.length) {
            for (const line of result.lines) {
                const textLine = line.words.map((w: { text: any; }) => w.text).join(' ');
...
export const anaysisTextLines = async (textLines: string[]): Promise<JSON> => {
    let result: any;
...
export const anaysisImage = async (file: any): Promise<string[]> => {
  let textLines: string[] = [];
  try {
    ...
    const computerVisionClient = new ComputerVisionClient(
        new ApiKeyCredentials({ inHeader: { 'Ocp-Apim-Subscription-Key': apiKey, 'content-type':"application/octet-stream"      }}), endpoint);
